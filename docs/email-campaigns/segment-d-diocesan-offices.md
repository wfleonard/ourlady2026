# Segment D — Diocesan Offices

Bulk B2B campaign targeting U.S. diocesan chanceries and their departmental offices (Hispanic ministry, faith formation, superintendents of Catholic schools, communications). Different persona than Segment A/B/C — decision-maker at scale, longer sales cycle, quotes rather than one-click checkout.

**Send window:** any time, but ideal is **October–February** — dioceses typically set annual budgets in the first quarter of the calendar year, and the Jubilee Year (through Oct 12, 2027) is a fresh, timely hook.

**Cadence:** three emails, spaced 7 days apart. Diocesan offices are slower to reply than parishes — leave more room between sends.

---

## Roles to target (in order)

1. **Director of Hispanic Ministry** or **Multicultural Affairs** (best fit for a Guadalupe-anchored initiative)
2. **Director of Faith Formation / Catechesis** (owns parish and school religious ed)
3. **Superintendent of Catholic Schools** (if pitching the school system rollout)
4. **Chancellor** (formal channel; slower but decisive)
5. **Bishop's secretary** (only if you know the bishop personally or by reputation)
6. **Director of Communications** (last resort — will forward, but doesn't decide)

## Personalization variables

| Variable | Example |
|----------|---------|
| `@firstname` | Fr. Miguel |
| `@diocesename` | Diocese of Trenton |
| `@city` | Trenton |
| `@state` | NJ |
| `@contactrole` | Director of Hispanic Ministry |
| `@parishcount` | 97 |

`@parishcount` is optional but adds real weight to the "one canvas per parish" pitch. The U.S. Conference of Catholic Bishops publishes parish counts by diocese — worth a couple of hours to fill in for your top targets.

## UTM tracking

```
?utm_source=openmoves&utm_medium=email&utm_campaign=diocesan-outreach-2026&utm_content=<email-number>
```

Distinct campaign ID so diocesan traffic is separable in GA from parish and school traffic.

---

## Email 1 — Introduction / Jubilee opportunity

**Send:** Day 0 (Tuesday or Wednesday morning)

**Subject:** A Jubilee-year Guadalupe canvas for every parish in the @diocesename

**Preview text:** Documented 24×36 tilma reproduction, bulk pricing, PO invoicing. Written quote in one business day.

**Body:**

Dear @firstname,

I'm writing to the @contactrole at the @diocesename because the Guadalupe Jubilee Year Pope Leo XIV opened on October 12 offers a natural occasion for a diocesan-wide initiative.

Primos Maternos produces a 24" × 36" canvas reproduction of the tilma of Saint Juan Diego. The image comes from a digital archive of the Sacred Original, which Cardinal Norberto Rivera Carrera, Archbishop Primate of Mexico, certified on December 12, 1998 as a faithful reproduction. The canvas is printed in the USA on archival stock at 287.5 pixels per inch, with nothing enlarged to fill it. Both Mexican certifications come with every order, and both are published in full at [primosmaternos.com/authenticity](https://primosmaternos.com/authenticity) so you can read them before you decide.

At the diocesan scale — for example, one canvas per parish across the @diocesename's @parishcount parishes — the pricing works out to a per-piece cost well below retail, on a single PO, with either single-address delivery to the chancery or split-shipping to each parish.

Common diocesan uses:

- A Jubilee-year gift to every parish, distributed with a bishop-signed card
- Catholic school system rollout — one canvas per religion classroom
- Hispanic Ministry office distribution at deanery meetings and retreats
- Ordination, installation, and jubilee gifts for clergy

Written quote within one business day of your reply. Purchase orders accepted from established diocesan offices — Net 30 standard.

[**See the diocesan program page →**](https://primosmaternos.com/dioceses?utm_source=openmoves&utm_medium=email&utm_campaign=diocesan-outreach-2026&utm_content=d-1)

Or reply to this email with an approximate quantity and target delivery date and I'll return a quote directly.

Thanks,
Peace Bill

William F. Leonard
Saxon Enterprises, Inc. — dba Primos Maternos
Tinton Falls, NJ
wfleonard@primosmaternos.com · 732-673-4260

---

## Email 2 — The image (value, not sales)

**Send:** Day 7

**Subject:** What science cannot explain about the tilma — a resource for your office

**Preview text:** A summary of the ophthalmology, chemistry, and infrared research on the image — useful for catechesis and Guadalupe-year programming.

**Body:**

Dear @firstname,

Not a sales email — a resource your Hispanic Ministry or Faith Formation office may find useful for Guadalupe-year catechesis, deanery programming, or a bulletin insert.

The 1936 pigment analysis by Nobel chemist Richard Kuhn: not animal, not mineral, not vegetable, no known source. The 1979 infrared analysis by Callahan and Smith: no paint, no brush strokes, no primer, painted in a single step. The thirteen human figures identified in the pupils of the eyes by a Cornell-trained IBM engineer using satellite-imagery algorithms. A NASA scientist's remark that "studying this image has been the most moving experience of my life."

Full write-up, with sources, about eight minutes of reading:

[**Read: What Science Cannot Explain About the Tilma →**](https://primosmaternos.com/blog/tilma-science?utm_source=openmoves&utm_medium=email&utm_campaign=diocesan-outreach-2026&utm_content=d-2a)

There is also a companion piece on the Aztec symbolism of the image — the mestiza face, the Nahui Ollin, the winter-solstice constellations on the mantle — which fits the Guadalupe-year theological formation cycle:

[**Read: A Codex in the Image →**](https://primosmaternos.com/blog/codex-symbols?utm_source=openmoves&utm_medium=email&utm_campaign=diocesan-outreach-2026&utm_content=d-2b)

Both are free to link in a diocesan newsletter or the Faith Formation LMS.

Diocesan bulk-order details remain here whenever they become relevant:

[Diocesan program →](https://primosmaternos.com/dioceses?utm_source=openmoves&utm_medium=email&utm_campaign=diocesan-outreach-2026&utm_content=d-2c)

Thanks,
Peace Bill

William F. Leonard
Saxon Enterprises, Inc. — dba Primos Maternos

---

## Email 3 — Quote framework / final ask

**Send:** Day 14

**Subject:** Six lines to get a diocesan quote from us

**Preview text:** Reply with these six items and we'll return written pricing within one business day.

**Body:**

Dear @firstname,

A last note.

If a diocesan-scale Guadalupe canvas program is on the @diocesename's radar at any point in the next 12 months of the Jubilee Year, the fastest way to a quote is a six-line reply to this email:

1. Approximate quantity
2. SKU mix (24"×36" framed, 24"×36" rolled, 36"×54" rolled, 12"×18" gallery wrap)
3. Ship-to (single chancery address, or split-ship to individual parishes)
4. Target delivery date
5. Whether a PO will be issued
6. Whether an initial sample canvas would be useful for internal review before the full order

We return written quotes within one business day. Standard fulfillment window for bulk orders is 3–4 weeks; split-shipping adds 1–2 weeks. Sample canvases can be shipped separately at retail pricing.

The Jubilee Year opened October 12, 2026 and runs through October 12, 2027. Dioceses that want the canvases in parish hands by the December 12 feast should aim for a PO by early November.

[**Diocesan program page →**](https://primosmaternos.com/dioceses?utm_source=openmoves&utm_medium=email&utm_campaign=diocesan-outreach-2026&utm_content=d-3)

Thanks,
Peace Bill

William F. Leonard
Saxon Enterprises, Inc. — dba Primos Maternos
Tinton Falls, NJ
wfleonard@primosmaternos.com · 732-673-4260

---

## Notes on operational readiness

The /dioceses page and the emails above assume:

- **You can fulfill 100+ canvas orders within 3–4 weeks.** If lead time is longer, adjust both the page and the emails.
- **You accept Net 30 POs from diocesan offices.** If credit-check first, note that in your quote response.
- **You have letterhead invoicing ready.** Diocesan accounting requires a formal invoice; a Stripe receipt alone won't clear a PO.
- **Volume pricing is real and defensible.** The page shows tiers (1–4, 5–24, 25–99, 100+) without dollar amounts — quotes are custom. Have a working discount schedule internally before the first quote request lands.

Flag if any of the above needs work before the first send.
